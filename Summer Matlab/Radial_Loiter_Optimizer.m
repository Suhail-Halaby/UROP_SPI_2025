...
alt_ceil = 120;
rms_out = 200;
max_clmgrad = deg2rad(20);
max_snkgrad = deg2rad(-20);
...
results = bayesopt(@(x) ObjectFun(x,theta,direction,num_stations,R_m,max_clmgrad,max_snkgrad), vars, ...
...
function [objective]  = ObjectFun (vars,theta,direction,num_stations,R_m,max_clmgrad,max_snkgrad)
    ...
    clm_violation = abs(max(0, [alt_grads(end),alt_grads] -  max_clmgrad)).^2;
    
    snk_violation = abs(min(0, [alt_grads(end),alt_grads] -  max_snkgrad)).^2;

    penalty = (sum(clm_violation)+sum(snk_violation)+sum(ro_violation)+sum(ri_violation))*1e3;
